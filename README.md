...
| `@all` в тексте (а также `@everyone`, `@все`, `@всех`, `@здесь`) | То же самое, но без слеша — достаточно написать `@all` в любом месте сообщения. |
...
```
Скрипт собирает свежий бинарь, заливает его на сервер под временным именем, делает атомарный `mv` (чтобы не упереться в `ETXTBSY` от запущенного процесса), рестартит сервис и показывает первые строки логов.

Если надо заодно обновить токен (например, BotFather отозвал старый):
```bash
./deploy.sh root@161.104.34.193 1234567890:AAA...new-token
```
Скрипт перепишет `/etc/tg-all-bot.env`, поставит `chmod 600`, и рестартит сервис. Токен передаётся через stdin SSH-сессии, поэтому в `ps`/history на сервере он не светится.

Если ARM-сервер: `ARCH=arm64 ./deploy.sh root@host`.
